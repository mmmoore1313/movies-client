// Import React so we can use JSX. Also import Component for our class component, and Fragment for Fragment
import React, { Component, Fragment } from 'react'
// Import Route from react-router for our SignIn and SignUp components
import { Route } from 'react-router-dom'
// Import the v4 function from the `uuid` package. Rename the `v4` function to be `uuid`
import { v4 as uuid } from 'uuid'

// This is a route we can only go to if we are signed in. Otherwise, it sends us to the root path "/"
import AuthenticatedRoute from './components/AuthenticatedRoute/AuthenticatedRoute'
// A component to show a user a message. The dismiss that message after 5 seconds
...
class App extends Component {
  ...
  // this method will reset the user state, to its initial value of null
  ...
  // variant - this is the bootstrap color to use as styling
  ...
  render () {
    ...
    return (
      <Fragment>
        ...
        {/* For each of our messages in msgAlerts: show an AutoDismissAlert component, which will show up on the screen and disappear after 5 seconds. */}
        ...
        <main className="container">
          ...
          <Route path='/sign-up' render={() => (
            // Show the SignUp component. Pass it the `msgAlert` prop to show a signed up
            // successfully message. And pass it `setUser` to automatically sign in.
            <SignUp msgAlert={this.msgAlert} setUser={this.setUser} />
          )} />
          {/* Same as /sign-up, but with the path of /sign-in, and with the component SignIn */}
          <Route path='/sign-in' render={() => (
            <SignIn msgAlert={this.msgAlert} setUser={this.setUser} />
          )} />
          {/* An AuthenticatedRoute is used just like a normal Route, except:
            1. The compoenet is called AuthenticatedRoute
            2. We pass it a `user` prop which it can use to tell if we are signed in */}
          ...
          {/* Add an AuthenticatedRoute to see all movies */}
